function [ cDat ] = get_condition_dat_for_save_byTrial(sID, condition, bl_dat, value_dat, tgt_dat, resps_dat, feed_dat)
...
cDat = zeros( sum( [prod(size(bl_dat)), prod(size(value_dat)), prod(size(tgt_dat)), prod(size(resps_dat)), prod(size(feed_dat))] )  , 7);
r = size(cDat, 1);
...
% time dimension
allDat = cat(2, bl_dat, value_dat, tgt_dat, resps_dat, feed_dat ); % so 14 + 40 + 40 + 39 + 39
...
% allocate to the overall cDat matrix (go through frequencies and channels and trials)
for iFreq = 1:size(allDat, 1)
    
    for iChan = 1:size(allDat, 4)
        
        for iTrials = 1:size(allDat, 3)
            
            tmp = allDat(iFreq, :, iTrials, iChan);   % get the data
            if iFreq == 1 && iChan == 1
                idx = 1:size(tmp, 2);
            else
