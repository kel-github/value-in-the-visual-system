...
function get_trial_log_v1(sub_num,date)
%%%%%%%%%%%% trial numbers
trials_per_cond = 120;
n_cond = 4;
trials_per_block = 60;
...
trial_matrix(:,5) = repmat([1 1 2 2]',(trials_per_block*n_cond)/2,1); %%%% cue direction
...
%%%%%%%%%%% now shuffle blocks
idx = 1:trials_per_block:trials_per_cond*n_cond;
idx = idx(randperm(length(idx)));
[rows,cols] = size(trial_matrix);
trial_log = zeros(rows,cols);

log_idx = 1:trials_per_block:trials_per_cond*n_cond;
for i = 1:length(idx)
    trial_log(log_idx(i):log_idx(i)+trials_per_block-1,:) = trial_matrix(idx(i):idx(i)+trials_per_block-1,:);
end
